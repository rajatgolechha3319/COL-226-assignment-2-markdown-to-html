...
fun trip_as(x:char list)=
  if length(x)<3 then false
    ...
    end;(* Function to check the occurence of triple asteriks as a look ahead *)
...
fun bold_n_italics( x:char list , y1:char list, z1:bool)=
  if length(x)<6 then reverse(y1)@x
    ...
    end; (*Implementation of the triple asterisks in the code by replacing them with bold and italics *)

fun bold_italics(x:char list)= bold_n_italics(x,[],false); (*Function to call the bold_n_italics function*)
...
fun double_as(x:char list)=
  if length(x)<2 then false
    ...
    end; (*Function to check the occurence of double asteriks as a look ahead *)
...
fun bold(x: char list,y1:char list,z1:bool)=
  if length(x)<4 then reverse(y1)@x
    ...
    end; (*Implementation of the double asterisks in the code by replacing them with bold *)

fun bold1(x:char list)= bold(x,[],false); (*Function to call the bold function*)
...
fun single_as(x:char list)=
  if length(x)<1 then false
    ...
    end; (*Function to check the occurence of single asteriks as a look ahead *)
...
fun italic(x:char list,y1:char list,z1: bool)=
  if length(x)<2 then reverse(y1)@x
    ...
    end; (*Implementation of the single asterisks in the code by replacing them with italics *)

fun italic1(x:char list)= italic(x,[],false);   (*Function to call the italic function*)
...
fun ul_helper(x:char list)=
  if length(x)<1 then false
    ...
    end; (*Function to check the occurence of single underscore as a look ahead *)
...
fun underline(x:char list,y1:char list,z1:bool)=
  if length(x)<2 then reverse(y1)@x
    ...
    end; (*Implementation of the single underscore in the code by replacing them with underline or spaces *)

fun ul_1(x:char list)= underline(x,[],false); (*Function to call the underline function*)
...
fun hash_detection(x:char list , y: char list, z: bool,count:int)=
  if length(x)<1 then reverse(y)@x
    ...
    in
      ...
      end;    (*Implementation of the hash in the code by replacing them with the heading tags*)

fun hash1(x:char list)= hash_detection(x,[],false,0); (*Function to call the hash_detection function*)
...
fun horizontal_ruling_valid(x:char list,blkquot: bool,spaces:bool ,count:int)=
  if length(x)<1 andalso count<3 then false
    ...
    end; (*Function to implement the hr tag when called by --- *)
...
fun hr_helper(x:char list,y:char list)=
  ...
  end; (*Function to implement the hr tag when called by --- with blockquotes *)


fun hr(x:char list)=
  if horizontal_ruling_valid(x,true,true,0) then hr_helper(x,[])
  else x; (*Function to call the hr function*)
...
fun valid_block( x : char list,count:int , flag:bool)=
    ...
    end; (*Function to check if the blockquote is valid or not*)

fun valid_blk(x:char list)= valid_block(x,0,true); (*Function to call the valid_block function*)
fun empty_line( x: char list)=
    ...
    end; (*Function to check if the line is empty or not*)

fun block_list( x:char list , y:int)=
    if y=0 then x
    else block_list(tl(x),y-1); (*Function to remove the blockquote from the line*)

fun block_gen( x: string, y:int)=
    if y=0 then (x)
    else block_gen("<blockquote>"^x,y-1); (*Function to generate the blockquote tag*)
fun block_gen_2( x: string, y:int)=
    if y=0 then (x)
    else block_gen_2("</blockquote>"^x,y-1); (*Function to generate the closing blockquote tag*)
fun counting_blocks(x:char list, suffix: char list, prev_count: int, running_p:bool)=
    ...
    end; (*Function to count the number of blockquotes in the line and implement them as in markdown *)
...
fun counting_blocks_2(x:char list, suffix: char list,prev_count:int, running_p:bool)=
    ...
    end; (*Function to count the number of blockquotes in the line and implement them as in markdown with blockquotes part 2*)
...
fun email_block_valid(x:char list,count:int)=
  if x=[] then count
    ...
    end; (*Function to count the number of blockquotes in the beginning of line*)

fun blk_count(x:char list)= email_block_valid(x,0); (*Function to call the email_block_valid function*)
...
fun email_block_gen(xi:char list,prev_count:int, running_p:bool)=
  ...
  end; (*Function to count the number of blockquotes in the line and implement them as in email block_quotes *) 
...
fun link_extractor(x:char list,y:char list, read:bool)=
  if length(x)<1 then reverse(y)
    ...
    end; (*Function to extract the link from the line with the help of a lookahead *)
...
if length(x)<1 then reverse(y)
  ...
  end; (*Function to extract the word from the line with the help of a lookahead *)
...
fun resume(x:char list, y: int,z:bool)=
  if length(x)<1 then []
    ...
    end; (*Function to resume the line after extracting the link and the word *)
...
fun link(x:char list, y1:char list)=
  if length(x)<1 then reverse(y1)
    ...
    end; (*Function to implement the link in the line *)
...
fun table_helper(x:char list,y:int)=
  if length(x)<2 then 0
    ...
    end; (* Function to form a FSM for different table states 00=no_table,01=table_begin, 10=table_end 11=table_running*)
...
fun table_gen(x:char list,y1:char list,t_start:int)=
  if length(x)<1 then explode("<TR><TD>")@reverse(y1)@explode("</TD></TR>")@x
    ...
    else x (*Function to generate the table commands and tags *)
...
(*This is the main function which uses the textIO library to extract the lines one by one and then them parse
them through multiple functions, which would then in turn produce the final html code which would then be
written in the final output file *)